...
contract D4AProtocol is ID4AProtocol, Multicallable, Initializable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
        ...
        returns (uint256 tokenId)
    {
        ...
        // mint
        tokenId = D4AERC721(pi.nft).mintItem(msg.sender, tokenUri);
        {
            pi.nftTotalSupply++;
            ci.nft_tokens.push(tokenId);
            ci.nft_token_number++;
        ...
        returns (bytes32)
    {
        ...
        {
            CanvasStorage.CanvasInfo storage ci = canvasInfos[canvasId];
            ci.project_id = daoId;
            ci.canvas_uri = canvasUri;
            ci.index = canvasIndex + 1;
